export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["game","gridSize","CircuitGrid","Direction","Radians","Numbers","Draw","Vec2"], function(game,gridSize,CircuitGrid,Direction,Radians,Numbers,Draw,Vec2){return(
game(
  gridSize*17,
  (circuit) => {
    ...
    CircuitGrid.insert(circuit, [11, 1], Direction.UP, CircuitGrid.BOX, {
      func: (vec, item) => {
        ...
        if (diff < 0.01 && diff > -0.01) circuit.score++;
      },
      label: "🏁"
    });
...
(height=gridSize*6,onInit=()=>{}, onLoop=()=>{}) => {
  ...
  (function tick() {
    ...
    const hoverOverwritable = !hoverEntry || hoverEntry.type === CircuitGrid.WIRE;

    if (input.isButtonDown('left') || input.isKeyDown("enter") || input.isKeyDown(" ")) {
        CircuitGrid.insert(circuit, hoverIndex2D, hoverDirection);
      }
    if (hoverOverwritable) {
      if (input.isKeyDown('x')) {
        CircuitGrid.remove(circuit, hoverIndex2D);
        CircuitGrid.insert(circuit, hoverIndex2D, hoverDirection, CircuitGrid.BOX, { 
      ...
      if (input.isKeyDown('h')) {
        CircuitGrid.remove(circuit, hoverIndex2D);
        CircuitGrid.insert(circuit, hoverIndex2D, hoverDirection, CircuitGrid.BOX, { 
          func: (vec) => Vec2.leftMulBy4x4Matrix([[1/Math.sqrt(2), 1/Math.sqrt(2)], [1/Math.sqrt(2), -1/Math.sqrt(2)]], vec),
          label: 'H' 
        });
      }
      ...
      if (input.isKeyDown('m')) {
        ...
        CircuitGrid.insert(circuit, hoverIndex2D, hoverDirection, CircuitGrid.BOX, { 
          func: (vec) => {
            const probVec = [vec[0]**2, vec[1]**2];
            if (Math.random() < vec[0]**2) {
              return [1, 0];
            } else {
              return [0, 1];
            }
  ...
  main.variable(observer("Draw")).define("Draw", ["Vec2","Iterable","Direction","Numbers","Radians","Color"], function(Vec2,Iterable,Direction,Numbers,Radians,Color)
{
  ...
  const source = (w, gridSize, context) => {
    ...
    context.fillStyle = "black";
    context.beginPath();
    context.arc(...center, gridSize / 2, 0, Math.PI * 2, true); // Outer circle
    context.fill();
    context.closePath();
  ...
  main.variable(observer("Numbers")).define("Numbers", function()
{
  ...
  main.variable(observer("InputState")).define("InputState", ["HTMLElement","HTMLDocument"], function(HTMLElement,HTMLDocument){return(
class InputState {
...
md`** Thanks for taking a look! I like *Θbservable*!**<br>
> Special thanks to S.W. and D.B. for giving feedback while I built this.<br>
