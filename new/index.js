...
function draw() {
  if (canvas.getContext) {
    ...
    function ctx_draw() {
	  ...
	  //ctx.fillStyle = 'rgba(255,155,255,0.2)';
      ...
      for (j = 0; j < edges.length; j++) {
		...
		edge_start = rotatePointY(camera_rot_y, edge_start, [camera_x, camera_y, camera_z]);	//camera rotations
		...
		var start_dist_cam = distToCamPlane(edge_start[2]);
		var end_dist_cam = distToCamPlane(edge_end[2]);
		if (end_dist_cam > start_dist_cam) {	//make the point closer to the camera the edge_start
		...
		if (start_dist_cam > 0) {	//if the edge_start is in front of the camera
			
			if (end_dist_cam < 0) {	//if edge_end is behind the camera clip the line with the cam plane
			...
			end_dist_cam = distToCamPlane(edge_end_z);	//recalculate clipped line distance to cam plane.
			
			var projection_start_x = (edge_start[0]-camera_x)*horizontal_zoom/Math.abs(start_dist_cam)+horizontal_offset;	//camera coords, zooms, perspective, offsets
			...
			var temp = rotatePointZ(camera_rot_z, [projection_start_x, projection_start_y], [horizontal_offset, vertical_offset]);	//z rotations
      ...
      for (i = 0; i < vertices.length; i++) {
		...
		var point_dist_cam = distToCamPlane(point[2]);
...
function distToCamPlane(z){
	var zthing = z - camera_z;

	if (zthing > 0) {
		return -1*distBetPoints(0, 0, zthing);
	} else {
		return distBetPoints(0, 0, zthing);
	}
